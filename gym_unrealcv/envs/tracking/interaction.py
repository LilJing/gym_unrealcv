...
class Tracking(Navigation):
    ...
    def random_obstacles(self, objects, img_dirs, num, area, start_area, obstacle_scales):
        # np.random.seed(1) # fix obstacles
        sample_index = np.random.choice(len(objects), num, replace=False)
        for k, id in enumerate(sample_index):
            obstacle_scale = obstacle_scales[k]
            obstacle = objects[id]
            ...
            # scale
            self.set_obj_scale(obstacle, np.random.uniform(obstacle_scale[0], obstacle_scale[1], 3))
            # print('obstacle scale', np.random.uniform(obstacle_scale[0], obstacle_scale[1], 3))
            # location
